...
def lambda_handler(event, context):
        ...
        df.columns = ["date_time", "branch", "customer", "order_content", "total_price", "payment_type", "credit_card_number"]
        # print(type(df))
        ...
        transaction_df = add_foreign_key_column(transaction_df, 'branch_id', 'branch', branch_name)
        ...
        product_dict = {'Regular Flavoured iced latte - Hazelnut': 2.75,
         'Large Latte': 2.45,
        ...
        product_df = pd.DataFrame(list(product_dict.items()), columns=['product_name', 'product_price'])
        ...
        def increase_transaction_id(conn, df, table_name):
                cursor = conn.cursor()
                cursor.execute(f"SELECT transaction_id FROM {table_name} ORDER BY transaction_id DESC LIMIT 1;")
